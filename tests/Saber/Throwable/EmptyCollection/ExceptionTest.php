...
namespace Saber\Throwable\EmptyCollection {
	...
	class ExceptionTest extends Core\AnyTest {
		...
		public function testBox($provided, $expected) {
			$p0 = Throwable\EmptyCollection\Exception::box($provided[0], $provided[1], $provided[2]);
		...
		public function testCompareTo($provided, $expected) {
			$p0 = Throwable\EmptyCollection\Exception::box($provided[0][0], $provided[0][1], $provided[0][2])->compareTo(Throwable\EmptyCollection\Exception::box($provided[1][0], $provided[1][1], $provided[1][2]));
			$e0 = $expected[0];
		...
		public function dataToString() {
			$data = array(
				array(array('Message', array(), 0), array('Saber\\Throwable\\EmptyCollection\\Exception [ 0 ]: Message ~ ')),
			);
			return $data;
		}
		...
		public function testToString($provided, $expected) {
			$p0 = Throwable\EmptyCollection\Exception::box($provided[0], $provided[1], $provided[2])->__toString();
			$e0 = $expected[0];
