...
namespace Saber\Data\IString {
	...
	final class ModuleTest extends Core\TypeTest {
		...
		public function testAll(array $provided, array $expected) {
			...
			$x = IString\Type::make($provided[0]);
			$y = IChar\Type::make($provided[1]);

			$p0 = $x->all(function(Core\Any $x, IInt32\Type $i) use ($y) {
				return $x->eq($y);
			});
			$e0 = $expected[0];
		...
		public function testAny(array $provided, array $expected) {
			...
			$p0 = $x->any(function(Core\Any $x, IInt32\Type $i) use ($y) {
				return $x->eq($y);
			});
			$e0 = $expected[0];
