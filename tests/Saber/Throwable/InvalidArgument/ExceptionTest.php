...
namespace Saber\Throwable\InvalidArgument {
	...
	final class ExceptionTest extends Core\Test {
		...
		public function testBox(array $provided, array $expected) {
			$p0 = Throwable\InvalidArgument\Exception::make($provided[0], $provided[1], $provided[2]);
			$e0 = new Throwable\InvalidArgument\Exception($expected[0], $expected[1], $expected[2]);

			$this->assertInstanceOf('\\InvalidArgumentException', $p0);
			$this->assertInstanceOf('\\Saber\\Core\\Type', $p0);
			$this->assertInstanceOf('\\Saber\\Throwable\\Runtime\\Exception', $p0);
			$this->assertInstanceOf('\\Saber\\Throwable\\InvalidArgument\\Exception', $p0);
		...
		public function testCompare(array $provided, array $expected) {
			$p0 = Throwable\InvalidArgument\Exception::make($provided[0][0], $provided[0][1], $provided[0][2])->compare(Throwable\InvalidArgument\Exception::make($provided[1][0], $provided[1][1], $provided[1][2]));
			$e0 = $expected[0];
		...
		public function testToString(array $provided, array $expected) {
			$p0 = Throwable\InvalidArgument\Exception::make($provided[0], $provided[1], $provided[2])->__toString();
			$e0 = $expected[0];
